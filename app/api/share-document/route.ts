...
export async function POST(req: NextRequest) {
  try {
    ...
    if (insertAccessError) {
      if (insertAccessError.code === "23505") {
        return NextResponse.json(
          {
            error:
              "This investor already has access to this document. They can open it from Shared with me or the link below.",
          },
          { status: 409 }
        );
      }
    ...
    const libraryUrl = `${siteUrl}/shared`;
    ...
    const sendResult = await resend.emails.send({
      ...
      html: `
        <div style="font-family: system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif; line-height: 1.5; color: #18181b;">
          ...
          <p style="margin: 16px 0 8px; font-size: 14px;">All documents shared with you (including older ones):</p>
          <p style="margin: 0 0 16px;">
            <a href="${libraryUrl}" style="color: #2563eb; font-weight: 600;">Open your shared library</a>
          </p>
          <p style="margin: 20px 0 0; font-size: 12px; color: #71717a;">If a button or link does not work, copy and paste into your browser:<br />
          Document: <span style="word-break: break-all;">${escapeHtml(viewUrl)}</span><br />
          Library: <span style="word-break: break-all;">${escapeHtml(libraryUrl)}</span></p>
        </div>
      `,
      text: [
        `${sharedBy} shared "${doc.title}" with you on Investor Portal.`,
        ``,
        `Open this document (sign in with this email):`,
        viewUrl,
        ``,
        `See all documents shared with you (past and current):`,
        libraryUrl,
      ].join("\n"),
    });
